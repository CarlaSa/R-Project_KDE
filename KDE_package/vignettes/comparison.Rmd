...
We follow RKurs Vorlesung 15 and run a simulation study to compare our bandwidth simulation methods for various functions and kernels. 

Sadly, we note that the current implementation of the Goldenschluger-Lepski method is very slow:
...
ggplot(data.frame(x=c(-3, 3)), aes(x=x)) + 
    ...
    scale_colour_identity("Function", guide="legend", 
                          ...
                          breaks = c("red", "black")) 
```
For this reason, we omit the Goldenschluger-Lepski method as it currently stands from the following simulation study. However, it could be easily included if the processing time became tractable.

We adapt the `compare()` function from the lecture:
...
```{r eval=FALSE, include=FALSE}
plot_comparison_res <- function(f=list(cauchy = pdfs$Cauchy),
                                res,
                                main=NA,
                                legend=NULL,
                                show_diff=TRUE,
...
```{r eval=FALSE, include=FALSE}
# a compare_ise for when we already have a res object
compare_ise_res <- function(res,
                        real_functions = list(cauchy=pdfs$Cauchy), 
                        n=1000,
                        try_kernels=list(gaussian=kernels$gaussian),
                        methods=list(CV="CV"),
...
```{r}
# Note: we should add at least one more kernel and one more function.
# and also increase the reps
try_kernels <- list(gaussian=kernels$gaussian)
methods <- list(CV="CV", PCO="PCO")
real_functions <- list(cauchy = pdfs$Cauchy)
reps <- 3
data_ise <- compare_ise_res(res1, real_functions, try_kernels=try_kernels, methods=methods, reps=reps)
```

Following the lecture, we find the MISE for our chosen methods, functions and kernels:
```{r}
data_ise %>%
